...
bitset<64>* des::aplicarPermutacionInicial(bitset<64>* textoPlano){
    ...
    for(int i = 0 ; i < 64 ; i++){
        (*textoPlanoAuxiliar)[i] = (*textoPlano)[permutacionInicial[i]-1];
    }
...
bitset<48>* des::aplicarExpansion(bitset<32>* texto){
    ...
    for(int i = 0 ; i < 48 ; i++){
        (*textoExpandido)[i] = (*texto)[seleccionEBit[i]-1];
    }
...
bitset<32>* des::aplicarPermutacionP(bitset<48>* texto){
    bitset<32>* bitsetAuxiliar = new (nothrow) bitset<32>();
    for(int i = 0 ; i < 32 ; i++){
        (*bitsetAuxiliar)[i] = (*texto)[permutacionP[i]-1];
    }
    return bitsetAuxiliar;
}
...
bitset<56>* des::aplicarPermutacionPC1(bitset<64>* llaveMaestra){
    ...
    for(int i = 0 ; i < 56 ; i++){
        (*bitsetAuxiliar)[i] = (*llaveMaestra)[permutacionPC1[i]-1];
    }
    return bitsetAuxiliar;
}

bitset<28>* des::obtenerCn(bitset<56>* llave){
...
bitset<28>* des::obtenerDn(bitset<56>* llave){
...
bitset<28>* des::aplicarCorrimientoCircular(bitset<28>* texto, int numeroDeCorrimientos){
    vector<bool> bitsAuxiliares(numeroDeCorrimientos);
    int j = numeroDeCorrimientos-1;
    for(int i = 0 ; i < numeroDeCorrimientos ; i++){
        bitsAuxiliares[i] = (*texto)[j];
        j--;
    }
    j = 27 - numeroDeCorrimientos;
    (*texto) = (*texto) << numeroDeCorrimientos | (*texto) >> (28-numeroDeCorrimientos);
    for(int i = 27 ; i > j ; i--){
        (*texto)[i] = bitsAuxiliares[i-27];
    }
    return texto;
}
...
bitset<48>* des::aplicarPermutacionPC2(bitset<56>* CnDn){
    bitset<48>* bitsetAuxiliar = new (nothrow)bitset<48>();
    for(int i = 0 ; i < 48 ; i++){
        (*bitsetAuxiliar)[i] = (*CnDn)[permutacionPC2[i]-1];
    }
    return bitsetAuxiliar;
}
