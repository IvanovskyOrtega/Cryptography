...
vector<uchar*> archivos::leerBytes(string nombre){
	...
	try{
		...
		if(input.is_open()){
			...
			while(numeroDeBytesLeidos > 0){
					input.read((char *)aux, 1);
					buffer.reserve(1);
					buffer.push_back(new uchar(*aux));
					numeroDeBytesLeidos --;
			}
			input.close();
		}
...
void archivos::escribirBytes(vector<uchar*> bytes, string nombre){
	try{
		int i = 0;
		ofstream archivoEncriptado (nombre, ios::binary);
		if(archivoEncriptado.is_open()){
			while(tamanio > 0){
					archivoEncriptado.write((const char*)bytes[i], 1);
					tamanio --;
					i++;
			}
