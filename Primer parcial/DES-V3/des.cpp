...
void des::aplicarPermutacionInicial(u64* textoPlano){
    u64 aux = *textoPlano;
    for(int i = 0 ; i < 64 ; i++){
        if(consultarBit(aux,permutacionInicial[i]))
...
void des::aplicarPermutacionInicialInversa(u64* textoPlano){
    u64 aux = *textoPlano;
    for(int i = 0 ; i < 64 ; i++){
        if(consultarBit(aux,permutacionInicialInversa[i]))
            poner1(textoPlano,63-i);
        else
            poner0(textoPlano,63-i); 
    }
...
u64 des::aplicarExpansion(u32 texto){
    u64 expansion = 0;
    for(int i = 0 ; i < 48 ; i++){
        if(consultarBit(texto,permutacionInicialInversa[i]))
...
int des::obtenerRenglon(u32 bloque,int numeroDeBloque);
int des::obtenerColumna(u32 bloque, int numeroDeBloque);
...
void des::leerLlaveBinaria(u64* llave, std::string cadenaLlave);
void des::leerLlaveHexadecimal(u64* llave, std::string cadenaLlave);
void des::leerLlaveASCII(u64* llave, std::string cadenaLlave);
...
void des::aplicarPermutacionPC2(u64* CnDn);
